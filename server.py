...
def get_message():
    ...
    email_body = message.as_json()['body']
    soup = BeautifulSoup(email_body, 'html.parser')
    text_without_tags = soup.get_text().replace('\n', ' ').replace(u'\xa0', u' ')
    text_without_tags = re.sub(' +', ' ', text_without_tags)
    print("text_without_tags is : ", text_without_tags)
    send_message = message.as_json(enforce_read_only=False)
    # enforce_read_only=False is required to return the full message object
    resonse = {"message" : send_message , "text_without_tags" : text_without_tags}
    return resonse
...
@flask_app.route('/nylas/ask-ai', methods=['POST'])
@is_authenticated
def ask_ai():
    """
    documentation
